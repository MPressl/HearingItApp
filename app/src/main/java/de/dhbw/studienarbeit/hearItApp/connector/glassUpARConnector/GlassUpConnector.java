 ...
 *  Created by Martin
 */
public class GlassUpConnector extends AbstractConnector {
     ...
     * Constructor
     */
    public GlassUpConnector(MainActivity activity) {
        super();
        ...
        this.glassAgent.setConnectionListener(new GlassUpAgentInterface.ConnectionListener() {
            @Override
            public void onConnectionChanged(int connectionStatus) {
                mainView.updateConnectionStatus(
                        GlassUpConnector.this,
                        connectionStatus == GlassUpServiceInterface
                                                    .CONNECTION_STATUS_CONNECTED);
            }
        });
        mainView.updateConnectionStatus(this, this.glassAgent.isConnected());
        ...
        if(!this.glassAgent.isConfigured()){
            /*Not configured*/
            Log.d(MainActivity.LOG_TAG,"App not configured, Scheduling send configure");
            /*Send the configuration message*/
            this.configHandler.sendEmptyMessage(1);
        }else{
            /*Already configured*/
            Log.d(MainActivity.LOG_TAG,"App Already configured");
            Toast.makeText(this.mainView.getApplicationContext(), "App already configured",
                    Toast.LENGTH_LONG)
                    .show();
        }
    }
